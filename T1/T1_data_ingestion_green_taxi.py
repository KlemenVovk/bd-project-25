...
import os
from utils.constants import RAW_DATA_ROOT, TASK1_OUT_ROOT, TASK1_GREENTAXI_SCHEMA, COLUMN_CONSISTENCY_NAMING_MAP, LOCATIONID_CENTROID_BOROUGH_CSV
...
def map_trip_type(value):
    # valid is 1 (street hail) or 2 (dispatch), else -1
    valid_trip_types = [1, 2]
    if float(value) in valid_trip_types:
        return int(value)
    return -1 # Invalid trip type
...
if __name__ == "__main__":
    ...
    print("Reading files...")
    files = sorted(glob(os.path.join(RAW_DATA_ROOT, "green_tripdata/*.parquet")))
    ...
    for i, f in tqdm(enumerate(files)):
        ...
        if 'ehail_fee' not in dfs[i].columns:
            dfs[i]['ehail_fee'] = 0.0
        ...
        dfs[i]['ehail_fee'] = dfs[i]['ehail_fee'].fillna(0.0).astype(float)
        ...
        dfs[i]['trip_distance'] = dfs[i]['trip_distance'].fillna(-1).astype(float)
        dfs[i]['trip_type'] = dfs[i]['trip_type'].map(map_trip_type, meta=('trip_type', 'int8'))
